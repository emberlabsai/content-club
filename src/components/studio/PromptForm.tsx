    ...
    <form onSubmit={handleSubmit} className="w-full max-w-3xl mx-auto animate-fade-in">
      ...
      {mode === GenerationMode.FRAMES_TO_VIDEO && (
        <div className="mb-6 p-6 bg-white/[0.02] border border-white/[0.06] rounded-xl flex flex-col items-center gap-4 animate-fade-in">
          ...
          <p className="text-[10px] text-stone/40 italic">Start frame required. End frame constrains the final composition.</p>
          <p className="text-[10px] text-amber-500/60 italic max-w-sm text-center">Note: Images with human faces may be rejected by Google's celebrity detection filter.</p>
          {startFrame && !endFrame && (
      ...
      {mode === GenerationMode.REFERENCES_TO_VIDEO && (
        <div className="mb-6 p-6 bg-white/[0.02] border border-white/[0.06] rounded-xl flex flex-col items-center gap-4 animate-fade-in">
          ...
          <p className="text-[10px] text-stone/30 italic max-w-sm text-center">
            Use for character identity, product consistency, or style reference.
            Cannot be combined with frames — use Frames or Reference mode separately.
          </p>
          <p className="text-[10px] text-amber-500/60 italic max-w-sm text-center">Images with faces may be rejected by Google's celebrity filter.</p>
        </div>
      )}
          ...
          <textarea
            ...
            placeholder={
              mode === GenerationMode.TEXT_TO_VIDEO
                ? 'Describe the video you want to create — include camera, lighting, audio cues...'
                : mode === GenerationMode.FRAMES_TO_VIDEO
                  ? 'Describe the motion and transformation between frames...'
                  : mode === GenerationMode.REFERENCES_TO_VIDEO
                    ? 'Describe the video using your references — be detailed about camera, action, lighting...'
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div>
          ...
          {isRef && (
            <p className="text-[9px] text-champagne/40 mt-1">Locked to Pro for references</p>
          )}
        </div>
